...
class Warning(Base):
    ...
    action_taken = Column(String, nullable=False)  # warning, mute_3h, mute_permanent
...
def add_filtered_word(word):
    session = get_session()
    try:
        existing_word = session.query(FilteredWord).filter_by(word=word).first()
        if existing_word:
...
def add_warning(user_id, word, chat_id, username=None, first_name=None):
    ...
    try:
        ...
        warning_count = session.query(Warning).filter_by(user_id=user_id).count()
